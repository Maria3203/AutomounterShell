...
void afisare_status_timer_lavinia(){
	...
	for ( int i = 0; i < nr_montari; i++){
		if (lista_montari[i].e_montat == 1){
			...
			printf("Punct: %s | Status: INACTIV (Asteapta cd)\n",lista_montari[i].destinatie);
		}
	}	
}
...
int montare_automata_lavinia(int index){
	...
	sprintf(comanda_sistem, "sudo mount --bind %s %s", lista_montari[index].sursa, lista_montari[index].destinatie);
	printf("Execut: %s\n", comanda_sistem);
...
int demontare(int index){
	char cmd[70];
	sprintf(cmd, "sudo umount %s", lista_montari[index].destinatie);
